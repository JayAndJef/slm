...
- `data/tokenizer-32k.json` — the real tokenizer (HF format, vocab 32000), at `slm.paths.TOKENIZER_PATH`. `notebooks/tokenizer.json` is the old 4096-vocab `SimpleTokenizer` artifact kept as the fallback (`paths.SIMPLE_TOKENIZER_PATH`) and still loaded by the notebook. `.gitignore` uses `data/*` + `!data/*.json` so the tokenizer ships but corpora don't. **The shipped 32k tokenizer predates the `train-tokenizer` val fix**: it was trained on `pool[0:10000]`, which contains the whole `pool[0:5000]` val split, so its merges saw val text and the recorded `val_bpb` is very slightly optimistic. `train-tokenizer` now samples `pool[5000:]`; re-running it produces a *different* tokenizer, hence a different `corpus_hash` and a full re-encode.
...
- **`TrainConfig.corpus_hash`** digests the tokenizer bytes **plus** `(dataset_mix, seed, sep, dataset_name)` and goes in the corpus filenames (`train_cosmo_5000+all_3c061ac0.npy` — the train name carries **both** doc counts, since `load_docs` slices train *after* the val docs and `n_val_docs` therefore moves the train slice too). Every one of those fails *silently* when it drifts — a swapped tokenizer leaves all ids in range and the loss curve looks normal; a changed mix or seed moves the val split with no signal. Changing any of them forces a visible re-encode instead of a wrong run. `n_workers` is deliberately excluded — it permutes shard concatenation order, not corpus contents — with the caveat that *order* is genuinely `n_workers`-dependent (`shard(contiguous=False)` interleaves differently for 8 vs 16), so two machines with different core counts write differently-ordered streams to the same filename.
